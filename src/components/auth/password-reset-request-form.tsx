...
export function PasswordResetRequestForm() {
  ...
  async function handleSubmit(values: PasswordResetRequestInput) {
    ...
    try {
      setSubmitting(true);
      // Placeholder: integrate Better Auth password reset request in auth-2b
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        <div className="space-y-4">
          ...
          {sent && !error && (
            <p className="text-sm font-medium text-green-600 dark:text-green-500">If that email exists, a reset link has been sent.</p>
          )}
        </div>
